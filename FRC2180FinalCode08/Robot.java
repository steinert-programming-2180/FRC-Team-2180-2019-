...
public class Robot extends TimedRobot {
  ...
  DoubleSolenoid endGame = new DoubleSolenoid(6, 7);
  Compressor compressor = new Compressor();

  int armTarget = 10;
  int wristTarget = 10;

  boolean continuePlacementRoutine = false;
  long startTime = 0;
  int stage = 0;
...
*/
  ...
  boolean grabPistonsOut = true;
  boolean hatchOutOut = false;

  int endgameState = 0; //0 is null, 1 is move to position and maintain, 2 goes down
  ...
  */
  ...
  public void periodicInit () {
    ...
    endGame.set(DoubleSolenoid.Value.kOff);
  ...
  public void periodicMotion () {
    ...
    setMotors(left, right, leftSpeed, rightSpeed, stick.getRawButton(2));
    ...
    if (stick4.getRawButton(10)) {
      armTarget = 609 - Constants.armOffset;
      wristTarget = 437 - Constants.wristOffset;
      motionMagic = true;
      endgameState = 1;
      // wristRight.setInverted(wristRightinverted);
      // wristRight.follow(wristLeft);
    ...
    if (stick4.getRawButton(7)) {
      armTarget = 509 - Constants.armOffset;
      wristTarget = 467 - Constants.wristOffset;
      motionMagic = true;
      endgameState = 1;
    }
    /*
    ...
    if(stick3.getRawButtonPressed(1)) {
      hatchMechanism.set(DoubleSolenoid.Value.kReverse);
    } if (stick4.getRawButtonPressed(1)) {
      hatchMechanism.set(DoubleSolenoid.Value.kForward);
    }
    
    if (stick4.getRawButtonPressed(2)) {
      hatchOut.set(DoubleSolenoid.Value.kForward);
    } if (stick4.getRawButtonPressed(3)) {
      hatchOut.set(DoubleSolenoid.Value.kReverse);
    }
    
    if (!stick4LastPress && stick4.getRawButton(4)) {  //Initiates automatic placement
      continuePlacementRoutine = true;
    } if (continuePlacementRoutine) {  //Actually does automatic placement
        if (stage == 0) { //Sets things up and extends pistons
          hatchOut.set(DoubleSolenoid.Value.kReverse);
          hatchOutOut = false;
          startTime = System.currentTimeMillis();
          stage = 1;
        } else if (stage == 1) { //Drops piston
          if ((System.currentTimeMillis() - startTime) > 250) {
            hatchMechanism.set(DoubleSolenoid.Value.kForward);
            grabPistonsOut = true;
            stage = 2;
          }
        } else if (stage == 2) { //Brings the assembly back in
          ...
          continuePlacementRoutine = false;
          stage = 0;
        }
    } stick4LastPress = stick4.getRawButton(4);

    if (stick4.getRawButtonPressed(11)) {
      endGame.set(DoubleSolenoid.Value.kReverse);
    } if (stick4.getRawButtonPressed(9)) {
    ...
    //   if ((armLeft.getClosedLoopError(0) < 30) && (wristLeft.getClosedLoopError(0) < 30)) {
    //     endgameState = 2;
    //   }
    ...
    SmartDashboard.putBoolean("Full Mech Out", hatchOutOut);
    SmartDashboard.putNumber("Endgame State", endgameState);
  }
